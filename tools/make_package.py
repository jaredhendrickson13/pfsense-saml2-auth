...
class MakePackage:
    ...
    def build_on_remote_host(self):
        ...
        build_cmds = [
            ...
            "python3 ~/build/pfsense-saml2-auth/tools/make_package.py --tag {t}".format(t=self.args.tag)
        ]
        ...
        if self.args.freebsd == 11:
            src = "{u}@{h}:~/build/pfsense-saml2-auth/pfSense-pkg-saml2-auth/pfSense-pkg-saml2-auth-{v}{r}.txz"
            src = src.format(
            ...
            src = "{u}@{h}:~/build/pfsense-saml2-auth/pfSense-pkg-saml2-auth/work/pkg/pfSense-pkg-saml2-auth-{v}{r}.txz"
            src = src.format(
                u=self.args.username,
                h=self.args.host,
                v=self.port_version,
    ...
    def __start_argparse__(self):
        ...
        parser = argparse.ArgumentParser(
            description="Build the pfSense SAML2 package on FreeBSD"
        )
        parser.add_argument(
            '--host', '-i',
            dest="host",
